...
public class X9180DozeService extends Service {
    private static final String TAG = "X9180DozeService";
    ...
    private X9180ProximitySensor mSensor;
    ...
    class X9180ProximitySensor implements SensorEventListener {
        ...
        public X9180ProximitySensor(Context context) {
		...
		if (DEBUG) Log.d(TAG, String.format("X9180DozeService: mInPocketTime = %d", mInPocketTime));
            }
            mSawNear = isNear;
        }
	    ...
	    if (DEBUG) Log.d(TAG, String.format("X9180DozeService: delta = %d", delta));
        ...
        public void enable() {
            if (mHandwaveGestureEnabled || mPocketGestureEnabled) {
		if (DEBUG) Log.d(TAG, "X9180DozeService: registerListener");
                mSensorManager.registerListener(this, mSensor, SensorManager.SENSOR_DELAY_NORMAL);
            }
        }

        public void disable() {
	    if (DEBUG) Log.d(TAG, "X9180DozeService: disable");
    ...
    public void onCreate() {
        if (DEBUG) Log.d(TAG, "X9180DozeService Started");
        mContext = this;
        mPowerManager = (PowerManager)getSystemService(Context.POWER_SERVICE);
        mSensor = new X9180ProximitySensor(mContext);
	...
	if (DEBUG) Log.d(TAG, String.format("X9180DozeService: loadPreferences - mHandwaveGestureEnabled = %b, mPocketGestureEnabled = %b",
    ...
    private SharedPreferences.OnSharedPreferenceChangeListener mPrefListener =
	    ...
	    if (DEBUG) Log.d(TAG, String.format("X9180DozeService: onSharedPreferenceChanged - mHandwaveGestureEnabled = %b, mPocketGestureEnabled = %b",
		mHandwaveGestureEnabled, mPocketGestureEnabled));
        }
    };
}
