...
LINEEND=" \\"
COUNT=`wc -l ../X9180/proprietary-files.txt | awk {'print $1'}`
DISM=`egrep -c '(^#|^$)' ../X9180/proprietary-files.txt`
COUNT=`expr $COUNT - $DISM`
for FILE in `egrep -v '(^#|^$)' ../X9180/proprietary-files.txt`; do
  COUNT=`expr $COUNT - 1`
  if [ $COUNT = "0" ]; then
    LINEEND=""
  fi
...
LOCAL_SRC_FILES := proprietary/lib/libOmxVdecHevc.so
