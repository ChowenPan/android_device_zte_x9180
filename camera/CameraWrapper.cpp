...
//    if (params.get(param_12)) {
//        videoMode = (!strcmp(params.get(param_12), "true"));
//    }
...
// White spots fixed in H116 firmware
/*
    if(id == 0) {
	if (params.get(param_12)) {
		videoMode = (!strcmp(params.get(param_12), "true"));
	...
	if (params.get(param_16)) {
		hdrMode = (!strcmp(params.get(param_16), param_17));
	}

	if (params.get(param_14)) {
		flashAuto = (!strcmp(params.get(param_14), "auto"));
		flashOn = (!strcmp(params.get(param_14), "on"));
	}

	if(!videoMode && (flashAuto || flashOn)) {
		params.set(param_9, "on");
		params.set(param_8, "1");
	}

	if(videoMode || hdrMode) {
		params.set(param_9, "off");
		params.set(param_8, "0");
	}
    }
*/
